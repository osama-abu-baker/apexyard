...
- **Every PR names the session that produced it** (fork-local) -- PR bodies must carry a `## Ref` section giving the agent session's name, e.g. `Ithbat IAM v0.2 | Orchestrator`. This portfolio runs many concurrent sessions committing under one git identity, so without it a merged PR records what changed but not who was driving — which is what made the duplicate-migration collision (ITH-347) an investigation rather than a lookup. Enforced through `.pr.required_sections` in `.claude/project-config.json`, the same gate that blocks a missing `## Glossary` — but that file is gitignored upstream, so the gate is **per-machine opt-in while the rule itself propagates**, and the hook can only check the section is present, not that its content is truthful. Treat it as belt-and-braces, not a guarantee. Full rule and both limits: @.claude/rules/pr-session-ref.md
